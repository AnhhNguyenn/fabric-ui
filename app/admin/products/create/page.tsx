...
export default function CreateProductPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white min-h-screen">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Tạo Sản Phẩm Mới (Demo)</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="p-6 border border-gray-100 rounded-2xl shadow-sm space-y-5 bg-white">
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-gray-700">Mô tả thiết kế</label>
            <textarea 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700">Giá (VNĐ) *</label>
              <div className="relative">
                <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input 
                  type="number" 
                  ...
                  value={price} 
                  onChange={e => setPrice(Number(e.target.value))} 
                  className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-rose-500" 
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <select 
                value={categoryId} 
                onChange={e => setCategoryId(e.target.value)} 
                className="w-full p-3 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-rose-500"
              >
                {categories.map(cat => (
                  <option key={cat._id} value={cat._id}>{cat.name}</option>
                ))}
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-gray-700">Hình ảnh sản phẩm *</label>
            <div className="flex items-center gap-4 border-2 border-dashed border-gray-200 p-6 rounded-2xl relative hover:bg-gray-50">
              <Image className="w-8 h-8 text-gray-400" />
              <div className="flex flex-col">
                 <span className="text-sm font-medium text-gray-600">Chọn ảnh demo</span>
              </div>
              <input 
                type="file" 
                multiple 
        ...
        <button 
          type="submit" 
          disabled={loading} 
          className="w-full bg-rose-600 text-white py-4 rounded-2xl font-bold uppercase tracking-widest shadow-lg shadow-rose-200 transition-all hover:bg-rose-700"
        >
          {loading ? 'Đang lưu demo...' : 'Lưu Sản Phẩm'}
        </button>
      </form>
    </div>
  );
